...
CraftWorld::CraftWorld()
{
    ...
    mainOptions.horizontalViewDistance = 2;
    ...
    playerSpawnPointPosition = Vector3(0.0f,64,0.0f);
...
void CraftWorld::UpdateBlocksLightNearby(const int xx, const int yy, const int zz) // Not finished, not used.
{
...
void CraftWorld::CalculateBlocksLightAtPos(const int xx, const int yy, const int zz, int iteration) // Not finished, not used.
{
...
void CraftWorld::SaveCompressedWorld(std::string  filename)
{
    ...
    if(mPigsize != 0)
    {
        ...
        float* data7 = new float[mPigsize]; // freezedTimerMax
        ...
        for(int k = 0; k < mPigsize; k++)
        {
            ...
            data7[k] = SavePig->freezedTimerMax;
        }
        ...
        gzwrite(saveFile, data7,sizeof(float)*(mPigsize));
...
void CraftWorld::LoadCompressedWorld(std::string  filename) // Загрузка мира
{
    ...
    if(worldVersion >= 3)
    {
        ...
        if(mPigsize != 0)
        {
            ...
            float* data7 = new float[mPigsize];
            ...
            gzread(saveFile, data7,sizeof(float)*(mPigsize));
            ...
            for(int g = 0; g < mPigsize; g++)
            {
                ...
                SavePig->HP = data5[g];
                SavePig->freezedTimerMax = data7[g];
...
void CraftWorld::GetSpecialBlock(int x,int y, int z,int &iVertex,SimpleMeshChunk* MeshChunk,block_t Block,bool transparent)
{
    ...
    if(blockType->blockModel == 5)
    {
        ...
        {
			if (mainOptions.fastRendering == false)
			{
				...
				BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16), Vector3(16, 16, 16), Vector3(16, 16, 0), Vector3(0, 16, 0));
	...
	if(blockType->blockModel == 12)
    {
        ...
        BuildWorldBlockPlane(blockType,x,y,z,iVertex,MeshChunk,Block,transparent,4,Vector3(0,15,16),Vector3(16,15,16),Vector3(16,15,0),Vector3(0,15,0));
        ...
        for (int y = StartY; y < CHUNK_SIZE + StartY; ++y)
        {
            for (int x = StartX; x < CHUNK_SIZE + StartX; ++x)
            {
				...
				if (GetBlock(x, y, z) == Fire::getID())
				{
					...
					if (GetBlock(x , y - 1, z) == WoodBlock::getID() || GetBlock(x , y - 1, z) == LeavesBlock::getID() || GetBlock(x , y - 1, z) == TNTBlock::getID() || GetBlock(x , y - 1, z) == BlackWoolBlock::getID() ||
						GetBlock(x , y - 1, z) == BlackWoolBlock::getID() || GetBlock(x , y - 1, z) == GrayWoolBlock::getID() || GetBlock(x , y - 1, z) == RedWoolBlock::getID() || GetBlock(x , y - 1, z) == PinkWoolBlock::getID() ||
						GetBlock(x , y - 1, z) == DarkGreenWoolBlock::getID() || GetBlock(x , y - 1, z) == GreenWoolBlock::getID() || GetBlock(x , y - 1, z) == BrownWoolBlock::getID() || GetBlock(x , y - 1, z) == YellowWoolBlock::getID() ||
						GetBlock(x, y - 1, z) == BlueWoolBlock::getID() || GetBlock(x , y - 1, z) == LightBlueWoolBlock::getID() || GetBlock(x , y - 1, z) == VioletWoolBlock::getID() || GetBlock(x , y - 1, z) == PastelWoolBlock::getID() ||
						GetBlock(x, y - 1, z) == DarkWoodBlock::getID() || GetBlock(x , y - 1, z) == WhiteWoodBlock::getID() || GetBlock(x , y - 1, z) == WoodenFence::getID() || GetBlock(x , y - 1, z) == OakHalfBlock::getID())
					{
						GetBlock(x, y, z) = 0;
					...
					/*if (GetBlock(x , y , z - 1) == WoodBlock::getID() || GetBlock(x , y , z - 1) == LeavesBlock::getID() || GetBlock(x , y , z - 1) == TNTBlock::getID() || GetBlock(x , y , z - 1) == BlackWoolBlock::getID() ||
						GetBlock(x , y , z - 1) == BlackWoolBlock::getID() || GetBlock(x , y , z - 1) == GrayWoolBlock::getID() || GetBlock(x , y , z - 1) == RedWoolBlock::getID() || GetBlock(x , y , z - 1) == PinkWoolBlock::getID() ||
						GetBlock(x , y , z - 1) == DarkGreenWoolBlock::getID() || GetBlock(x , y , z - 1) == GreenWoolBlock::getID() || GetBlock(x , y , z - 1) == BrownWoolBlock::getID() || GetBlock(x , y , z - 1) == YellowWoolBlock::getID() ||
						GetBlock(x , y , z - 1) == BlueWoolBlock::getID() || GetBlock(x , y , z - 1) == LightBlueWoolBlock::getID() || GetBlock(x , y , z - 1) == VioletWoolBlock::getID() || GetBlock(x , y , z - 1) == PastelWoolBlock::getID() ||
						GetBlock(x , y , z - 1) == DarkWoodBlock::getID() || GetBlock(x , y , z - 1) == WhiteWoodBlock::getID() || GetBlock(x , y , z - 1) == WoodenFence::getID() || GetBlock(x , y , z - 1) == OakHalfBlock::getID())
					{
						GetBlock(x, y, z) = 0;
            ...
            for(int i = 0; i < waterFrameSize; i++)
            {
                for(int j = 0; j < waterFrameSize; j++)
                {
                    ...
                    TextureManager::Instance()->setColour(waterFrameSize*15+i,
                                                          ...
                                                          textureTerrainId);
					//fire
					TextureManager::Instance()->setColour(waterFrameSize * 13 + i,
														  waterFrameSize * 15 + j,
